...
interface ReportData {
  ...
  createdAt: Timestamp;
  status: "pending" | "verified" | "resolved" | "rejected" | "disputed";
}
...
export function UserProfile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const userReports = reportsSnapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() } as ReportData)
        );
  ...
  const handleUpdateProfile = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const regionName =
        regions.find((r) => r.code === psgc.regionCode)?.name || "";
      const provinceName =
        provinces.find((p) => p.code === psgc.provinceCode)?.name || "";
      const cityName = cities.find((c) => c.code === psgc.cityCode)?.name || "";
      const barangayName =
        barangays.find((b) => b.code === psgc.barangayCode)?.name || "";
      ...
      <div className="container mx-auto max-w-4xl px-4 py-6">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 flex flex-wrap gap-4">
                <button
                  ...
                  className="bg-[#0B1F8C] text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-900 transition-colors"
                >
                  <Settings size={16} />
                  Edit Profile
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow p-5 flex flex-col items-center">
            <div className="text-4xl font-bold text-[#0B1F8C]">
              {stats.total}
            </div>
        ...
        <div className="bg-white rounded-xl shadow">
          <div className="p-5 border-b border-gray-100">
            ...
            <p className="text-sm text-gray-500">
              Your recent report submissions
            </p>
          </div>
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h3 className="font-medium text-gray-800 capitalize">
                          {report.incidentType} Report
                        </h3>
                        <div className="flex items-center gap-1">
                          {statusInfo.icon}
                          <span
                            className={`text-xs font-medium ${statusInfo.textClass}`}
                          >
                            {statusInfo.label}
                          </span>
                        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleUpdateProfile}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="pt-2 border-t">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label
                        htmlFor="address.psgc.regionCode"
                        className="block text-sm font-medium text-gray-700 mb-1"
                      >
                        Region
                      </label>
                      <select
                        id="address.psgc.regionCode"
                        value={editFormData.address?.psgc?.regionCode || ""}
                        onChange={handleEditChange}
                    ...
                    <div>
                      <label
                        htmlFor="address.psgc.provinceCode"
                        className="block text-sm font-medium text-gray-700 mb-1"
                      >
                        Province
                      </label>
                      <select
                        id="address.psgc.provinceCode"
                        value={editFormData.address?.psgc?.provinceCode || ""}
                        onChange={handleEditChange}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white"
                        required
                        disabled={
                          !editFormData.address?.psgc?.regionCode ||
                    ...
                    <div>
                      <label
                        htmlFor="address.psgc.cityCode"
                        className="block text-sm font-medium text-gray-700 mb-1"
                      >
                        City/Municipality
                      </label>
                      <select
                        id="address.psgc.cityCode"
                        value={editFormData.address?.psgc?.cityCode || ""}
                        onChange={handleEditChange}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white"
                        required
                        disabled={
                          !editFormData.address?.psgc?.provinceCode ||
                          addressLoading.cities
                        }
                      >
                        <option value="" disabled>
                          {addressLoading.cities ? "Loading..." : "Select City"}
                        </option>
                        {cities.map((c) => (
                    ...
                    <div>
                      <label
                        htmlFor="address.psgc.barangayCode"
                        className="block text-sm font-medium text-gray-700 mb-1"
                      >
                        Barangay
                      </label>
                      <select
                        id="address.psgc.barangayCode"
                        value={editFormData.address?.psgc?.barangayCode || ""}
                        onChange={handleEditChange}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white"
                        required
                        disabled={
                          !editFormData.address?.psgc?.cityCode ||
              ...
              <div className="p-4 bg-gray-50 rounded-b-xl flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-[#0B1F8C] text-white rounded-lg hover:bg-blue-900 disabled:bg-gray-400 flex items-center gap-2"
                  disabled={isUpdating}
                >
